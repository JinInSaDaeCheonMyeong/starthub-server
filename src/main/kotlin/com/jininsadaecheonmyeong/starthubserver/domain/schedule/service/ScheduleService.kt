...
class ScheduleService(
    ...
    @Transactional
    fun createSchedule(userId: Long, request: ScheduleRequest) {
        val user = userRepository.findById(userId)
            .orElseThrow { UserNotFoundException("찾을 수 없는 유저") }
        val announcement = announcementRepository.findById(request.announcementId)
    ...
    fun getSchedulesByMonth(userId: Long, date: LocalDate): List<ScheduleResponse> {
    ...
    @Transactional(readOnly = true)
    fun getSchedulesByDate(userId: Long, date: LocalDate): List<AnnouncementSummaryResponse> {
