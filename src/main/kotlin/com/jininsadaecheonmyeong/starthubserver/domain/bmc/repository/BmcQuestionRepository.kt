...
interface BmcQuestionRepository : JpaRepository<BmcQuestion, UUID> {
    fun findByIdAndUser(id: UUID, user: User): Optional<BmcQuestion>
    fun findBySessionIdAndUser(sessionId: String, user: User): Optional<BmcQuestion>
    fun findAllByUserOrderByCreatedAtDesc(user: User): List<BmcQuestion>
    fun existsBySessionIdAndUser(sessionId: String, user: User): Boolean
}